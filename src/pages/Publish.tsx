  ...
  return (
    <PageShell>
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        <div>
          <Typography.Text style={{ color: palette.muted, textTransform: 'uppercase' }}>Publish</Typography.Text>
          <Typography.Title level={3} style={{ margin: 0 }}>发布书籍</Typography.Title>
          <Typography.Text type="secondary">填写书籍信息并上传图片，完善后即可发布。</Typography.Text>
        </div>
        {!loggedIn && (
          <Alert
            type="warning"
             ...
             <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
               <Upload
                 ...
                 customRequest={async ({ file, onError, onSuccess }) => {
                   ...
                   const fileName = realFile?.name || `image-${Date.now()}`;
                   ...
                   try {
                     ...
                     setImages((prev) => [...prev, { uid: String(Date.now()) + fileName, name: fileName, url: data.url }]);
